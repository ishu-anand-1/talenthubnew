...
const Talent = () => {
  ...
  useEffect(() => {
    const fetchVideos = async () => {
      try {
        const res = await api.get("/videos/get-all-video"); // ✅ Correct endpoint
        // Normalize for filtering
        const normalized = res.data.map((video) => ({
          ...video,
          category: video.category || "N/A",
          genre: video.genre?.toLowerCase() || "all",
          level: video.level?.toLowerCase() || "all",
          title: video.title || "",
  ...
  useEffect(() => {
    const filtered = videos.filter((v) => {
      ...
      const matchesGenre =
        filters.genre === "All" || v.genre === filters.genre.toLowerCase();
      const matchesLevel =
        filters.level === "All" || v.level === filters.level.toLowerCase();
      const matchesSearch =
        v.title.toLowerCase().includes(search.toLowerCase()) ||
        v.description.toLowerCase().includes(search.toLowerCase());

      return matchesCategory && matchesGenre && matchesLevel && matchesSearch;
    });
  ...
  return (
    <div className="px-6 py-10 min-h-screen">
      <h1 className="text-3xl font-bold mb-6 text-center text-red-100">
        🎭 Talent Gallery
      </h1>
